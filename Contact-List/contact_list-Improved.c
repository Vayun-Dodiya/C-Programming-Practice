...
int main()
{
    ...
    while (1)
    {
        ...
        {
            char str[100];
            FILE *fp = fopen("contact_list.copilat-ai-improved.txt", "r");
        ...
        again:
            ...
            FILE *fp = fopen("contact_list.copilat-ai-improved.txt", "a");
        ...
        if (op == 4)
        {
            ...
            // Lower the searchName for case-insensitive search
            ...
            FILE *check_fp = fopen("contact_list.copilat-ai-improved.txt", "r");
            if (check_fp == NULL)
            {
                printf("File not found\n");
                break;
            ...
            {
                if (strstr(check_line, "==>") != NULL)
                {
                    char check_nameLine[300];
                    if (fgets(check_nameLine, sizeof(check_nameLine), check_fp))
                    {
                        if (strstr(check_nameLine, lowerSearch) != NULL)
                        {
                            exists = 1;
                            break;
                        }
            ...
            // Proceed with removing since name exists
            int match = 0;
            char line[300];
            int found = 0;
            int skipping = 0;
            ...
            while (fgets(line, sizeof(line), fp))
            {
                ...
                if (strstr(line, "==>") != NULL)
                {
                    ...
                    if (fgets(nameLine, sizeof(nameLine), fp))
                    {
                        if (strstr(nameLine, lowerSearch) != NULL)
                        {
                            printf("\n--- Match Found ---\n");
                            match = 1;
            ...
            remove("contact_list.copilat-ai-improved.txt");
            rename("temp.txt", "contact_list.copilat-ai-improved.txt");
        }
        if (op == 5)
        {
            int edit;
            ...
            // Proceed with editing since name exists
            char line[300];
            int found = 0;
            int skip_lines = 0;
            FILE *fp = fopen("contact_list.copilat-ai-improved.txt", "r");
                        ...
                        if (strstr(nameLine, lowerSearch) != NULL)
                        {
                            ...
                            if (edit == 1)
                            {
                                ...
                                if (digit != 10) {
                                    ...
                                    // For simplicity, continue with invalid
                                }
                                fprintf(temp, "%s", line);
                                fprintf(temp, "%s", nameLine);
            ...
            {
                // This should not happen since we checked exists, but just in case
                printf("Unexpected error: Contact not found during update.\n");
                remove("temp.txt");
            }
        }
